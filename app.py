...
# تصميم الـ UI ليكون جذاب (Minimalist & Dark Theme)
st.markdown("""
    <style>
...
if st.button("Run Intelligence Analysis"):
    if not url:
        ...
        with st.spinner("Processing through Tayel AI Neural Engine..."):
            # تشغيل الـ Agent
            result = asyncio.run(run_agent(url, lang_code))
            
            if "❌ Error" in result:
                # لو فيه خطأ، هنظهره بشكل شيك برضه عشان الـ Screen
                st.error(result)
            else:
                st.balloons()
